...
class BlackjackGame:
    ...
    def play_round(self, round_num):
        ...
        if player_value == 21:
            print(f"\n{Colors.GREEN}★ BLACKJACK! ★{Colors.RESET}")
            print(f"{Colors.GREEN}Player wins immediately!{Colors.RESET}")
...
def run_tcp_server(tcp_port, wifi_ip):
    """
    Run the TCP server to accept client connections.
    
    Args:
        tcp_port (int): Port to listen on
        wifi_ip (str): Wi-Fi adapter IP address to bind to
    """
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server_socket.bind((wifi_ip, tcp_port))
    server_socket.listen(5)
    
    actual_port = server_socket.getsockname()[1]
    print(f"{Colors.GREEN}TCP server listening on {wifi_ip}:{actual_port}{Colors.RESET}")
...
def run_udp_broadcast(tcp_port, wifi_ip):
    """
    ...
    # Bind to Wi-Fi adapter to ensure broadcasts go through Wi-Fi
    broadcast_socket.bind((wifi_ip, 0))
...
def main():
    ...
    try:
        run_tcp_server(actual_tcp_port, local_ip)
    except KeyboardInterrupt:
